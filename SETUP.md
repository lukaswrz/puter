...
```bash
nix run git+https://hack.moontide.ink/pingfisher/zap.git /path/to/disk
...
nixos-install --no-root-password --flake git+https://hack.moontide.ink/pingfisher/puter.git#hostname
```
